      ...
      if (status != "authenticated") {
        if (query?.error == "Password-fail") {
          ...
          setSuccess({
            status: "error",
            message:
              "Your password is invalid, try again or contact support.",
          });
          alert(message);
        }
        if (query?.error == "Email-exists") {
          ...
          setSuccess({
            status: "error",
            message: "Your email does not exist, create a new user or contact support.",
          });
          alert(message);
        }
      }
            ...
            >
              <Dialog.Panel className="relative bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-lg sm:w-full sm:p-6">
                <div className="p-5">
                  ...
                  <form className="space-y-6" onSubmit={form.onSubmit((values) => handleSubmit(values))}>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center">
                        <Checkbox
                          ...
                          {...form.getInputProps("saveLogin", {type: "checkbox",})}/>
                      </div>
                    ...
                    <div>
                      <Button type="submit" className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-[#F87171] hover:bg-[#F87171] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#F87171]" loading={rollingButton}>
                        Log in
                      </Button>
                    </div>
                    <div>
                  ...
                  <div className="mt-6">
                    ...
                    <div className="mt-6 grid grid-cols-1 gap-1">
                      <div>
                        <a onClick={() => signIn("facebook")} className="w-full inline-flex justify-center hover:cursor-pointer py-2 px-4 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-gray-500 hover:bg-gray-100" >
